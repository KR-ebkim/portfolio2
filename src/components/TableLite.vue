          ...
          <table class="vtl-table vtl-table-hover vtl-table-bordered vtl-table-responsive vtl-table-responsive-sm"
            ...
            <thead class="vtl-thead">
              <tr class="vtl-thead-tr">
                <th class="vtl-thead-th vtl-checkbox-th">
                </th>
                <th v-for="(col, index) in columns" class="vtl-thead-th" :class="col.headerClasses" :key="index" :style="
                  Object.assign(
                    {
            ...
            <template v-if="rows.length > 0">
              <tbody v-if="isStaticMode" class="vtl-tbody"
                ...
                <template v-for="(rows, groupingIndex) in templateRows" :key="groupingIndex">
                  <tr v-if="groupingKey != ''" class="vtl-tbody-tr vtl-group-tr">
                    <td class="vtl-tbody-td vtl-group-td">
                      <div class="flex">
                        <div v-if="hasGroupToggle" class="animation">
                          <a class="cursor-pointer" @click="toggleGroup($event, groupingIndex)">▼</a>
                        </div>
                  ...
                  <tr v-for="(row, i) in rows" :key="row[setting.keyColumn] ? row[setting.keyColumn] : i"
                    ...
                    <td v-for="(col, j) in columns" :key="j" class="vtl-tbody-td" :class="col.columnClasses"
              ...
              <tbody v-else :set="(templateRows = groupingKey == '' ? [rows] : groupingRows)">
                <template v-for="(rows, groupingIndex) in templateRows" :key="groupingIndex">
                  ...
                  <tr v-for="(row, i) in rows" :name="'vtl-group-' + groupingIndex"
                    :key="row[setting.keyColumn] ? row[setting.keyColumn] : i" class="vtl-tbody-tr" :class="
                      typeof rowClasses === 'function' ? rowClasses(row) : rowClasses
                    " @click="$emit('row-clicked', row)">
                    <td class="vtl-tbody-td">
                      <div>
                        <!--아이콘-->
                        <v-btn @click="favorite(row)" variant="text" icon color="#A6A6A6">
                          <v-icon color="#0054D1" v-if="row?.value">mdi-star</v-icon>
