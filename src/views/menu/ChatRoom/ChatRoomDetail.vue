<script setup>
import { ref, watch } from 'vue';
import { useRoute } from 'vue-router'

const route = useRoute()
</script>

<template>

    <div style="width: 100%; height:100%; background-color: #EFF3F6;">
        <v-container style="padding: 45px;">
            <div flat class="d-flex align-center">
                <span style="font-size:18px" class="font-weight-bold">
                    Chat Room
                </span>
                <v-spacer>
                </v-spacer>
                <v-btn icon flat color="transparent"><v-icon>mdi-home-outline</v-icon></v-btn>
                <v-icon>mdi-chevron-right</v-icon> <v-btn style="font-size:14px" variant="text">{{ route.name }}</v-btn>
            </div>
            <v-card flat rounded="lg">
                <div class="d-flex align-center">
                    <v-btn flat style="font-family: Spoqa Han Sans Neo;
font-size: 15px;
font-weight: 700;
line-height: 18px;
letter-spacing: 0em;
"><v-icon>mdi-arrow-left</v-icon>An Electric Circuit Team Part Leader</v-btn>
                    <v-spacer></v-spacer>
                    <div class="d-flex align-center ma-4 mt-6">
                        <div class="mr-2">
                            <div style="font-family: Spoqa Han Sans Neo;
font-size: 13px;
font-weight: 700;
line-height: 16px;
letter-spacing: 0em;
text-align: left;
">
                                Mitchell Williamson
                            </div>
                            <div style="font-family: Spoqa Han Sans Neo;
font-size: 12px;
font-weight: 400;
line-height: 14px;
letter-spacing: 0em;
text-align: right;
">
                                Techross 1.part
                            </div>
                        </div>
                        <v-avatar size="30">
                            <v-img cover
                                src="data:image/jpeg;base64,"></v-img>
                        </v-avatar>
                    </div>
                </div>
                <v-card flat style=" background-color: #EFF3F6;" rounded="0" class="mx-4 ">


                    <!--메세지 보낸 분-->
                    <v-card flat color="#0054D1" class="text-white pa-2 mx-4 my-2 ml-auto" rounded="lg" max-width="320"
                        style="font-family: Spoqa Han Sans Neo;
font-size: 15px;
font-weight: 400;
line-height: 20px;
letter-spacing: -0.23999999463558197px;
text-align: left;
">현재 BWMS 장치에 긴급알람이 발생하는데, <br> 이에 대한 원인을 모르겠습니다. </v-card>
                    <v-card flat color="#0054D1" class="text-white pa-2 mx-4 my-2 ml-auto" rounded="lg" max-width="320"
                        style="font-family: Spoqa Han Sans Neo;
font-size: 15px;
font-weight: 400;
line-height: 20px;
letter-spacing: -0.23999999463558197px;
text-align: left;
">현재 기기 알림소리가 어디서 나는지 알 수 없습니다. </v-card>
                    <v-card flat color="#0054D1" class="text-white pa-2 mx-4 ml-auto" rounded="lg" max-width="320"
                        style="font-family: Spoqa Han Sans Neo;
font-size: 15px;
font-weight: 400;
line-height: 20px;
letter-spacing: -0.23999999463558197px;
text-align: left;
">긴급 A/S 연결 부탁드립니다. </v-card>
                    <div class="d-flex ml-auto" style="display: block; width: 398px;">
                        <span class="d-flex align-end pb-2" style="font-size:13px; color:gray;">오후 1:00</span>
                        <v-card flat color="#0054D1" class="text-white pa-2 ma-2" rounded="lg" max-width="320"
                            style="font-family: Spoqa Han Sans Neo;
font-size: 15px;
font-weight: 400;
line-height: 20px;
letter-spacing: -0.23999999463558197px;
text-align: left;
">알람코드 A-110에서 오류가 발생한 것 같습니다. <br> 해당 사항에 대한 가이드 안내 부탁드립니다.
                        </v-card>
                    </div>




                    <!--메세지 받는 분-->
                    <v-card flat color="white" class="pa-2 mx-2" rounded="lg" max-width="320" style="font-family: Spoqa Han Sans Neo;
font-size: 15px;
font-weight: 400;
line-height: 20px;
letter-spacing: -0.23999999463558197px;
text-align: left;
">선박 BUSAN PIONEER 확인했습니다.<br>
                        [A-123] (T.S Temperature High) 1회 발생<br>
                        [A-304] FMU Flow Rate Low 2회 발생<br>
                        [A-721] Valve Opened 2회 발생<br>
                    </v-card>
                    <div class="d-flex">
                        <v-card flat color="white" class="pa-2 ma-2" rounded="lg" max-width="320" style="font-family: Spoqa Han Sans Neo;
font-size: 15px;
font-weight: 400;
line-height: 20px;
letter-spacing: -0.23999999463558197px;
text-align: left;
">발생원인은<br>
                            ECU내 압력 or 온도가 기준치보다 높음 (표준 압력 : 4.5 bar 미만, 표준 온도 : 45℃ 미만)<br>
                            Pressure Switch, Temperature Switch 또는 ESJ Board 불량으로 분석했습니다.<br>
                        </v-card>
                        <span class="d-flex align-end pb-2" style="font-size:13px; color:gray">오후 2:00</span>
                    </div>
                </v-card>

                <v-card rounded="0" color="#BFBFBF" class="pa-2 mx-4 mb-4">
                    <v-text-field append-inner-icon="mdi-send" variant="outlined" density="compact"
                        style="color:#0054D1;background-color: white;" hide-details="">
                    </v-text-field>
                </v-card>
            </v-card>
        </v-container>
    </div>
</template>



<style>

</style>