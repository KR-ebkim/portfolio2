...
<template>
    ...
    <div style="width: 100%; height:100%; background-color: #EFF3F6;">
        <v-container style="padding: 45px;">
            ...
            <div class="d-flex">
                <div>
                    <v-card width="250px" rounded="lg" style="border:1px #BFBFBF solid" flat>
                        <v-list v-model:opened="open">
                            <v-list-group value="ECS">
                                <template v-slot:activator="{ props }">
                                    <v-list-item v-bind="props" title="ECS" active-class="activeClass1"></v-list-item>
                                </template>

                                <v-list-group value="CPC">
                                    <template v-slot:activator="{ props }">
                                        <v-list-item v-bind="props" title="1 CPC"></v-list-item>
                                    </template>

                                    <v-list-group>
                                        <template v-slot:activator="{ props }">
                                            <v-list-item v-bind="props" title="1.1 a"
                                                active-class="activeClass3"></v-list-item>
                                        </template>
                                        <v-list-item v-for="([title], i) in CPC" :key="i" :title="title" :value="title"
                                            active-class="activeClass2"></v-list-item>
                                    </v-list-group>
                                </v-list-group>

                                <v-list-group value="PDE">
                                    <template v-slot:activator="{ props }">
                                        <v-list-item v-bind="props" title="2 PDE"></v-list-item>
                                    </template>

                                    <v-list-group>
                                        <template v-slot:activator="{ props }">
                                            <v-list-item v-bind="props" title="2.1 a"
                                                active-class="activeClass3"></v-list-item>
                                        </template>
                                        <v-list-item v-for="([title], i) in PDE" :key="i" :title="title" :value="title"
                                            active-class="activeClass2"></v-list-item>
                                    </v-list-group>
                                </v-list-group>
                            </v-list-group>
                ...
                <div class="ml-5">
                    ...
                    <v-card width="65vw" style="background-color:white; border: 1px #EBEBEB solid;" flat>
                        <v-card class="pa-2" style="background-color:#F9F9F9; border-bottom: 1px #EBEBEB solid;" flat>
                            <div class="text-center d-flex justify-center">
                                ...
                                <v-btn class="primary mr-2" flat
                                    @click="currentPage = currentPage > 1 ? currentPage - 1 : 1" variant="outlined"
                                    color="grey" style="background-color:white">Prev</v-btn>
...
<!--<v-list active-class="activeClass" :items="eCSList" item-title="text" item-value="id" width="275px"></v-list>--> 
</template>
